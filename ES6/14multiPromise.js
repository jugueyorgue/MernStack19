...
let createSession = (sessionName) => {
    return new Promise((resolve, reject) => {
        ...
        setTimeout(() => {
            if (isSuccess) {
                resolve(`${sessionName} session completed successfully.`);
            } else {
                reject(`${sessionName} session failed.`);
            }
        }, 2000); 
    });
};
...
let morningSession = createSession("Morning");
let afternoonSession = createSession("Afternoon");
let eveningSession = createSession("Evening");


Promise.all([morningSession, afternoonSession, eveningSession])
    .then(results => console.log("All sessions succeeded:", results))
    .catch(error => console.log("One or more sessions failed:", error));