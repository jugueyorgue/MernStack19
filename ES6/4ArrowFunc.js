...
let Accountfunct = (name = "jugue", balance = 0) => {
    
    return {
        name,
        balance,
        info: () => `Account Name: ${name}, Balance: $${balance}`
       
    };
};

// Usage
let account1 = Accountfunct("Cathy", 1500);
console.log(account1.info()); 
// Arrow Function without return

let Accountfunct2 = (name = "Yorlein", balance = 0) => ({
    name,
    balance,
    info: () => `Account Name: ${name}, Balance: $${balance}`
});
...
// Usage
let account2 = Accountfunct2("Yorguedi", 1000);
